...
Deno.serve(async (req) => {
  ...
  // Self-service actions are only valid for accounts created by the admin
  // invitation flow. A normal signed-in intern must never be able to call the
  // onboarding action and promote their own account.
  if (SELF_SERVICE.has(action)) {
    const { data: invitedUser, error: invitedUserError } = await admin.auth
      .getUser(jwt);
    if (invitedUserError || !invitedUser?.user) {
      return json({ error: "Invalid invitation session." }, 401);
    }
    const metadata = invitedUser.user.user_metadata || {};
    if (metadata.invited_as !== "admin") {
      return json({ error: "This account does not have an admin invitation." }, 403);
    }
  }
  ...
  try {
    ...
    if (action === "setup_complete") {
      ...
      if (meRes?.user?.user_metadata?.admin_activated !== true) {
        return json({ error: "Complete administrator registration first." }, 403);
      }
